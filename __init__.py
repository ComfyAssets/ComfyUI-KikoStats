...
try:
        ...
        if hasattr(server, 'PromptServer') and hasattr(server.PromptServer, 'instance'):
            ...
            try:
                ...
                async def get_monitor_reference(request):
                    ...
                    return web.Response(text="window.kikoStatsGlobalMonitor = " + str(id(monitor)) + ";", 
    ...
    try:
        ...
        if install_execution_hooks():
            print("\033[92m[KikoStats] Execution hooks installed for per-node tracking\033[0m")
        else:
            print("\033[93m[KikoStats] Could not install execution hooks - per-node tracking disabled\033[0m")
    except Exception as e:
        print(f"\033[93m[KikoStats] Execution hooks error: {e}\033[0m")
    
    print("\033[92m[KikoStats] Continuous monitoring initialized on ComfyUI startup\033[0m")
except Exception as e:
    print(f"\033[93m[KikoStats] Warning: Could not start continuous monitoring: {e}\033[0m")
